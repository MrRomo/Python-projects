...
class Azure:
    ...
    def add_face(self, imageBytes, person_id):
        ...
        if self.verify_detection(responseDetection):
            self.attributes = responseDetection[0]
            targetFace = self.get_face_rectangle(responseDetection[0])
            imgObject = Img(imageBytes)
            if person_id:
                try:
                    CF.large_person_group_person_face.add(imgObject, self.largePersonGroupId,person_id,target_face=targetFace)
                    print("Added face!!")
                except requests.exceptions.ConnectionError:
    ...
    def get_all_names(self):
        ...
        groupInfo = []
        try:
            persons = CF.large_person_group_person.list(self.largePersonGroupId)
